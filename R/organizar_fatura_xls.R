...
origem <- "data-raw/fatura.csv"

fatura <- read.csv2(origem,
                     header=TRUE, 
                     fill=TRUE, 
                     fileEncoding="latin1", 
                     sep=","
...
fatura <- fatura |> 
  dplyr::rename(data = "...1",
         ...
         valor = "...5")
fatura <- dplyr::select(fatura, 1, 2, 3, 5)
...
fatura <- fatura |> 
  dplyr::mutate(valor = as.numeric(str_replace_all(valor, ",", ".")),
...
writexl::write_xlsx(fatura, "data/fatura.xlsx")
